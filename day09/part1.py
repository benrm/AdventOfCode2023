...
for i in range(len(histories)):
    ...
    while True:
        if history.child is None:
            history.array.append(0)
        else:
            history.array.append(history.child.array[len(history.child.array)-1]+history.array[len(history.array)-1])
        if history.parent is not None:
            history = history.parent
        else:
            break
    _sum += history.array[len(history.array)-1]
print("Sum:", _sum)
