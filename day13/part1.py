...
for i in range(len(grids)):
    vertical_reflection = find_vertical_reflection(grids[i])
    horizontal_reflection = find_vertical_reflection([ [ grids[i][x][y] for x in range(len(grids[i])) ] for y in range(len(grids[i][0])) ])
